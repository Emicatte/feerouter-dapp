...
/**
 * RSendCCIPSender — Cross-chain token sender via Chainlink CCIP.
 *
 * Deploy: 1 per chain sorgente (Base, Ethereum, Arbitrum, etc.)
 *
 * Flow (sendCrossChain):
 *   1. Utente approva token al sender
 *   2. Sender prende token, applica fee 0.5%
 *   3. Fee -> treasury, resto -> CCIP bridge
 *   4. CCIP trasporta token alla chain destinazione
 *   5. RSendCCIPReceiver distribuisce al recipient
 *
 * Flow (swapAndBridge):
 *   1. Pull tokenIn dal sender
 *   2. Swap tokenIn → tokenOut via Uniswap V3
 *   3. Fee 0.5% di tokenOut → treasury
 *   4. tokenOut netto → CCIP bridge → destination chain
 *
 * Fee CCIP: pagata in ETH nativo (msg.value) dall'utente.
 * Fee RSend: 0.5% sul token trasferito.
 */
...
contract RSendCCIPSender is Ownable, ReentrancyGuard {
    ...
    function sendCrossChain(
        ...
        IRouterClient.EVM2AnyMessage memory message = IRouterClient.EVM2AnyMessage({
            ...
            data: abi.encode(recipient),   // il receiver finale
            tokenAmounts: tokenAmounts,
            feeToken: address(0),          // paga in ETH nativo
            extraArgs: ""                  // default: 200k gas limit
        });
    ...
    //
    //  L'utente manda ETH. Viene wrappato, swappato, e bridgato.
    //  msg.value deve coprire: amountIn (per lo swap) + CCIP fee
    // ══════════════════════════════════════════════════════════════════════
    function swapETHAndBridge(
        ...
        emit CrossChainSwapAndBridge(
            messageId, destinationChainSelector,
            msg.sender, recipient,
            address(0), tokenOut,  // address(0) = ETH nativo
            amountIn, amountOut, netAmount, fee
        );
    }
    ...
    function estimateFee(
        ...
        IRouterClient.EVMTokenAmount[] memory tokenAmounts = new IRouterClient.EVMTokenAmount[](1);
        tokenAmounts[0] = IRouterClient.EVMTokenAmount({ token: token, amount: netAmount });
