/**
 * useBackendCallback.ts
 * 
 * Hook per inviare i dati di TransactionStatus.tsx al backend RPagos.
 * Da usare nel componente padre quando la TX è completata (phase === 'done').
 * 
 * Esempio d'uso:
 * 
...
export function useBackendCallback() {
  return async (data: CallbackPayload) => {
    ...
    // Calcola HMAC: fiscal_ref|tx_hash|amount|currency|timestamp
