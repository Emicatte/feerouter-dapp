...
const cinematicT: Transition = { duration: 0.6, ease: EASE }
...
const formV: Variants = {
  ...
  exit:   { opacity: 0, y: -12, scale: 0.98, transition: { duration: 0.35, ease: EASE } },
}
...
// ═══════════════════════════════════════════════════════════
//  HERO TITLE — grid overlay cross-fade
// ═══════════════════════════════════════════════════════════
function HeroTitle({ view, setView }: { view: View; setView: (v: View) => void }) {
  ...
  return (
    <div style={{ display: 'grid', placeItems: 'center', height: 160 }}>
      <AnimatePresence>
        {view === 'send' && (
          <motion.div key="h-send" variants={heroV} initial="enter" animate="center" exit="exit" transition={cinematicT} style={{ gridArea: '1/1' }}>
            <div style={base}>
        ...
        {view === 'swap' && (
          <motion.div key="h-swap" variants={heroV} initial="enter" animate="center" exit="exit" transition={cinematicT} style={{ gridArea: '1/1' }}>
            <div style={base}>
        ...
        {view === 'command' && (
          <motion.div key="h-cmd" variants={heroV} initial="enter" animate="center" exit="exit" transition={cinematicT} style={{ gridArea: '1/1' }}>
  ...
  return (
    <>
      ...
      <main style={{
        ...
        {/* Nav pills */}
        <div style={{ display: 'flex', gap: 3, marginBottom: 32, background: C.surface, borderRadius: 14, padding: 3, border: `1px solid ${C.border}` }}>
          {([{ key: 'send' as View, label: '↗ Send' }, { key: 'swap' as View, label: '🔄 Swap' }, { key: 'command' as View, label: '🎛 Command Center' }]).map(v => (
            <button key={v.key} onClick={() => setView(v.key)} style={{
              ...
              opacity: v.key === 'command' ? 0.7 : 1,
            }}>{v.label}</button>
          ))}
        </div>

        {/* Content — stabilized container */}
        <div style={{ width: '100%', minHeight: 400, display: 'flex', justifyContent: 'center', alignItems: 'flex-start' }}>
          <AnimatePresence mode="wait">
            {view === 'send' && (
              <motion.div key="c-send" variants={formV} initial="enter" animate="center" exit="exit" style={{ width: '100%', maxWidth: 480 }}>
                <TransferForm />
              </motion.div>
            )}
            {view === 'swap' && (
              <motion.div key="c-swap" variants={formV} initial="enter" animate="center" exit="exit" style={{ width: '100%', maxWidth: 440 }}>
                <SwapModule onSwapComplete={() => {}} />
              </motion.div>
            )}
            {view === 'command' && isConnected && (
              <motion.div key="c-cmd" variants={formV} initial="enter" animate="center" exit="exit" style={{ width: '100%', maxWidth: 800 }}>
            ...
            {view === 'command' && !isConnected && (
              <motion.div key="c-nocon" variants={formV} initial="enter" animate="center" exit="exit" style={{ textAlign: 'center', padding: 40, background: C.surface, borderRadius: 20, border: `1px solid ${C.border}`, maxWidth: 380 }}>
